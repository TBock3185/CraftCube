...
"""
    ...
    version 12 - lpg 3 nov 2008 added option to specify precision of arc to
               linear conversion (arc was approximated with line for each
               20 degress). Note: not all fonts have arc definitions.
               - lpg 14 sept 2014 updated some defaults
"""
version = '12'
...
#            segs = int((end_angle - start_angle) / 20) + 1
            segs = int((end_angle - start_angle) / float(self.Arc.get())) + 1
...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.st14 = Label(self.EntryFrame, text='Arc-Approximation(deg/seg)')
        self.st14.grid(row=14, column=0)
        self.ArcVar = StringVar()
        self.ArcVar.set('+15.0')
        self.Arc = Entry(self.EntryFrame, width=15, textvariable = self.ArcVar)
        self.Arc.grid(row=14, column=1)


        self.st15 = Label(self.EntryFrame, text='Postamble')
        ...
        self.Postamble.grid(row=15, column=1)

        self.st16 = Label(self.EntryFrame, text='Text Orientation')
        self.st16.grid(row=16, column=0)
        self.MirrorVar = IntVar()
        self.MirrorVar.set(0)
        self.FlipVar = IntVar()
        self.FlipVar.set(0)
        Checkbutton(self.EntryFrame, text='Mirrored', variable=self.MirrorVar, command=self.DoIt).grid(row=16, column=1,sticky=W)
        Checkbutton(self.EntryFrame, text='Flipped', variable=self.FlipVar,command=self.DoIt).grid(row=16, column=1,sticky=E)
        ...
        self.quitButton.grid(row=17, column=0, sticky=S)
...
#=======================================================================
    def DoIt(self):
        ...
        Arc =   float(self.Arc.get())
        self.Arc.configure( bg = self.NormalColor )
        if Arc <= 0.0 or Arc > 45.0:
            self.Arc.configure( bg = 'red' )
            return
        ...
        font = parse(self,file)          # build stroke lists from font file
        file.close()
